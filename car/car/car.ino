...
#define BUZZER_PIN       10   // Buzzer Port
...
void moveHead(){
  if (movingHead &&
      (movingStatus == MOVING_FORWARD || movingStatus == MOVING_FORWARD_RIGHT || movingStatus == MOVING_FORWARD_LEFT)) {
    switch(headStatus){
        case HEAD_STOPPED:
...
void doActionFromKey(int cmdKey){
  switch(cmdKey){
    case KEY_UP:
      accelerate(+20);
      break;
...
void buzz(){
  ...
  delay (100) ;
  digitalWrite (BUZZER_PIN, 0) ; //no tone
}
